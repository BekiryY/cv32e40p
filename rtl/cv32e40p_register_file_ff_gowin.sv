...
module cv32e40p_register_file #(
 ...
 // 1. Multiplex the Write Ports
  // Priority multiplexer: Port B overrides Port A
  logic                  unified_we;
  logic [ADDR_WIDTH-1:0] unified_waddr;
  logic [DATA_WIDTH-1:0] unified_wdata;

  assign unified_we    = we_a_i | we_b_i;
  assign unified_waddr = we_b_i ? waddr_b_i : waddr_a_i;
  assign unified_wdata = we_b_i ? wdata_b_i : wdata_a_i;

// 2. Protect R0 (Address 0)
  // RISC-V requires x0 to be hardwired to 0. We disable writes to address 0.
  logic final_we;
  assign final_we = unified_we & (unified_waddr != 0);

  // Internal wires for RAM read outputs
  logic [DATA_WIDTH-1:0] ram_out_a;
  logic [DATA_WIDTH-1:0] ram_out_b;
  logic [DATA_WIDTH-1:0] ram_out_c;
